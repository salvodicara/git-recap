...
_git_recap() {
  ...
  case "$prev" in
    ...
    # Subcommands complete right after the command itself — "recap" covers
    # invocation through git ("git recap <TAB>"), where COMP_CWORD is 2.
    git-recap|recap)
      if [[ "$cur" != -* ]]; then
...
complete -c git-recap -n "__fish_seen_subcommand_from completion" -x -a "bash zsh fish"
